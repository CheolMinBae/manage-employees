...
// Excel 날짜 시리얼 번호를 날짜로 변환 (수정된 버전)
const excelDateToJSDate = (serial) => {
  // Excel의 날짜 시리얼 번호를 JavaScript Date로 변환
  // Excel에서 1은 1900년 1월 1일이지만, 실제로는 1900년 1월 0일부터 시작
...
// 중복 스케줄 확인
const checkDuplicateSchedule = async (userId, date, start, end) => {
  try {
...
// 직접 데이터베이스에 스케줄 저장
const createScheduleDirectDB = async (scheduleData) => {
  try {
    // 중복 확인
...
const processScheduleData = async () => {
  ...
  for (let rowIndex = 1; rowIndex < scheduleData.length; rowIndex++) {
    ...
    for (let dateIndex = 0; dateIndex < dates.length; dateIndex++) {
      ...
      // 직접 데이터베이스에 스케줄 저장
      const dbResult = await createScheduleDirectDB(scheduleEntry);
      ...
      if (dbResult.success) {
        ...
        console.log(`    ❌ 스케줄 저장 실패: ${dbResult.error}`);
      }
    }
  }
...
const main = async () => {
  try {
    console.log('=== 📋 스케줄 직접 DB 저장 스크립트 시작 ===');
    ...
    console.log(`⚠️  중복: ${results.duplicates.length}개 스케줄`);
    ...
    if (results.duplicates.length > 0) {
      ...
      results.duplicates.slice(0, 5).forEach((item, index) => {
        console.log(`${index + 1}. ${item.name} - ${item.date} ${item.time}`);
      });
      if (results.duplicates.length > 5) {
        console.log(`... 그리고 ${results.duplicates.length - 5}개 더`);
      }
    }
    ...
    console.log(`총 저장된 스케줄 수: ${totalSchedules}`);
